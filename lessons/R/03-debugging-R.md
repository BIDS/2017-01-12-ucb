...
because later errors are often caused by the initial error. It's
common to have a string of many errors, which looks daunting, caused
...
(your function is not looking for objects where you think it is) and
we are using a global variable that we did not mean to.

Another basic strategy is to build up code in pieces (or tear it back
in pieces to a simpler version). This allows you to isolate where the
error is occurring.

If you've written your code modularly (functions are not nested within
each other) with lots of functions, you can test individual
...
variable and thereby decipher what could be going wrong. We have
